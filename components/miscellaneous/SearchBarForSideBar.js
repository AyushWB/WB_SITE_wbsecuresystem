...
const SearchBarForSideBar = ({
  ...
  const searchHandler = () => {
    ...
    const slug = value.replaceAll(pattern, "-").toLowerCase();
    if (value) {
      router.push(`/${slug}/${selectedCity}/all`);
    }
  };
  ...
  const onSuggestionClick = (suggestion) => {
    ...
    vendorObject.forEach((obj) => {
      Object.keys(obj).forEach((key) => {
        if (key.toLowerCase() === suggestion.toLowerCase()) {
          matchedVendor = key;
          vendorMatchedSlug = obj[matchedVendor];  // <-- Fixed here from venueMatchedSlug
        }
      });
    });

    if (venueMatchedSlug) {
      router.push(`/${selectedCity}/${venueMatchedSlug}`);
    } else if (vendorMatchedSlug) {
      router.push(`/${selectedCity}/${vendorMatchedSlug}`);
    } else {
      const pattern = /[^a-zA-Z0-9]/g;
      const slug = suggestion.toLowerCase().replaceAll(pattern, "-");
      router.push(`/${slug}/${selectedCity}/all`);
  ...
  return (
    <Container showSuggestions={showSuggestions}>
      <Autosuggest
        ...
        getSuggestionValue={(suggestion) => suggestion}
        renderSuggestion={(suggestion, { query, isHighlighted }) => (
          <Suggestion className={`suggestion-${suggestionsList.indexOf(suggestion)}`}>
            {suggestion}
          </Suggestion>
        )}
        shouldRenderSuggestions={() => true}
