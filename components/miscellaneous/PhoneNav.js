...
function PhoneNav() {
  ...
  const openLeadsModel = useCallback((e, type = "click") => {
    ...
    localStorage.setItem("hasTriggered", "true");
    localStorage.setItem("lastTriggerTime", new Date().getTime().toString());
  }, [router.asPath, setLeadFormData, setIsLeadsModelOpen]);

  // Optimized menu determination logic
  const determineActiveMenu = useCallback((path) => {
    if (!venue_list || !venueCategories || !vendor_list || !vendorCategories) {
      return "home"; // Default to home if data is not available
    }
  ...
  useEffect(() => {
    ...
    // Memoized active menu determination
    if (venue_list && venueCategories && vendor_list && vendorCategories) {
      setActiveMenu(determineActiveMenu(router.asPath));
    }
  ...
  }, [router.asPath, venue_list, venueCategories, vendor_list, vendorCategories, determineActiveMenu]);
  ...
  }, [hasTriggered, triggerCount, openLeadsModel]);
