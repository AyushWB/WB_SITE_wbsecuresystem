...
const SearchBarVenue = ({
  ...
  const searchHandler = () => {
    ...
    if (value == "Wedding Transportation / Vintage Cars") {
      setValue("Wedding Transportation Vintage Cars");
    }
    if (value == "Makeup Artists") {
      setValue("Top Makeup Artists");
    }
    ...
    if (value == "Invitation Card") {
      setValue("Wedding Invitation Card");
    }
    var pattern = /[^a-zA-Z0-9]/g;
    const slug = value.replaceAll(pattern, "-");
    console.log(slug);
  ...
  suggestion = "Wedding Decorators";
    ...
    vendorObject.forEach((obj) => {
      Object.keys(obj).forEach((key) => {
        if (key.toLowerCase() == suggestion.toLowerCase()) {
          matchedVendor = key;
          vendorMatchedSlug = obj[matchedVendor]; // Correct this line
        }
      });
    });
    if (venueMatchedSlug) {
...
// const SearchBarVenue = ({ suggestions, selectedCity, venueObject, vendorObject }) => {
//   const [value, setValue] = useState("");
//   const [suggestionsList, setSuggestionsList] = useState([]);
//   const [showSuggestions, setShowSuggestions] = useState(false);
//   const [isInputFocused, setIsInputFocused] = useState(false);
...
//   const buildAndPushUrl = (rawSuggestion) => {
//     const suggestion = rawSuggestion
//       .replace("5 Start Wedding Hotels", "5 Star Wedding Hotels")
//       .replace("Wedding Transportation / Vintage Cars", "Wedding Transportation And Vintage Cars")
//       .replace("Photographers / Videography", "Best Wedding Photographers")
//       .replace("Makeup Artists", "Top Makeup Artists")
//       .replace("Mehendi Artists", "Best Mehndi Artists")
//       .replace("Choreographers", "Wedding Choreographer")
//       .replace("Decorators/Florists", "Wedding Decorators");
...
//     let url = "";
//     if (matchedSlug && matchedType) {
//       url = `/${selectedCity}/${matchedSlug}`;
//     } else {
//       const slugified = suggestion.toLowerCase().replace(/[^a-zA-Z0-9]/g, "-");
//       url = `/${slugified}/${selectedCity}/all`;
//     }
...
//   const onSuggestionClick = (suggestion) => {
//     setShowSuggestions(false);
//     buildAndPushUrl(suggestion);
//   };

//   const getSuggestions = (inputValue) => {
//     const trimmed = inputValue.trim().toLowerCase();
//     if (!trimmed) return suggestions.filter((s) => s.toLowerCase().startsWith("a"));
...
//   const debouncedFetch = debounce(onSuggestionsFetchRequested, 800);
...
//     const noSuggestions = suggestionsList.length === 0 && value.trim() !== "" && isInputFocused;

//     return (
//       <SuggestionsContainer ref={ref} {...rest} showSuggestions={showSuggestions}>
//         <div style={{ maxHeight: "200px", overflowY: "auto" }}>
//           {showSuggestions && noSuggestions && (
//             <div
//               style={{
...
//       setIsInputFocused(false);
//       setTimeout(() => {
//         if (!isInputFocused) onSuggestionsClearRequested();
//       }, 100);
//     },
//     onKeyPress: (e) => {
//       if (e.key === "Enter") {
...
//         renderSuggestion={(s) => <Suggestion>{s}</Suggestion>}
//         shouldRenderSuggestions={() => true}
//         inputProps={inputProps}
//         onSuggestionSelected={(_, { suggestion }) => onSuggestionClick(suggestion)}
//         theme={{
//           container: { position: "relative", width: "100%" },
//           suggestionsContainer: { backgroundColor: "#fff", zIndex: 9999 },
//           suggestionHighlighted: { backgroundColor: "#bf9539" },
//         }}
//       />
//       <div className="search-btn" onClick={searchHandler}>
//         <FaSearch color="var(--secoundary-color)" className="search-icon" />
