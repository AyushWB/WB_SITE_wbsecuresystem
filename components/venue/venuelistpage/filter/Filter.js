...
import BudgetRangeSlider from "@/components/miscellaneous/filter/BudgetSlider";
import getLocalitiesCat from "@/lib/request/getlocalities/getLocalitiesCat";
import { CheckFilterCity } from "@/components/miscellaneous/filter/Checkfiltercity";
...
function Filter({ filterQuery, localities, venueCategories, city, category, locality }) {
    ...
    const facilitiesItem = [
        { id: 0, name: "Food Provided By Venue", slug: "food-provided-by-venue" },
        { id: 1, name: "Outside Food Allowed", slug: "outside-food-allowed" },
        { id: 2, name: "Alcohol Allowed", slug: "alcohol-allowed" },
        { id: 3, name: "Outside Alcohol Allowed", slug: "outside-alcohol-allowed" },
        { id: 4, name: "Music Allowed Late", slug: "music-allowed-late" }
    ];

    const meallist = [
        { name: "Vegetarian Only", id: 1, slug: "veg" },
        { name: "Non-Vegetarian Only", id: 2, slug: "non-veg" }
    ];
    ...
    const [selectedLocalities, setSelectedLocalities] = useState(filterQuery.multi_localities?.split(",") || []);
    const [selectedCities, setSelectedCities] = useState(filterQuery.multi_cities?.split(",") || []);
    ...
    const [searchValue, setSearchValue] = useState(filterQuery.search_value);
    const [cityLocalities, setCityLocalities] = useState([]);
    ...
    function handleApplyFilter() {
        const query = {
            guest,
            per_budget: perBudget.join(","),
            per_plate: perPlate.join(","),
            multi_localities: selectedLocalities.join(","),
            multi_cities: selectedCities.join(","),
            food_type: selectedFoodPreference[0],
        };

        router.push({
            pathname: `/${selectedCategories[0] || "banquet-halls"}/${city}/${localities.length >= 1 ? "all" : locality}`,
            query,
        });
    }
        ...
        <Wrapper>
            <div className="filters">
                ...
                <CheckFilter
                    ...
                    type="venue"
                    name="Localities"
                    list={selectedLocalities}
                    setList={setSelectedLocalities}
                    handleApplyFilter={handleApplyFilter}
                />

                <BudgetRangeSlider
                    perBudget={perBudget}
                    setPerBudget={setPerBudget}
                    perPlate={perPlate}
                ...
                <RedioFilter
                    ...
                    items={meallist}
                    list={selectedFoodPreference}
                    setList={setSelectedFoodPreference}
                    handleApplyFilter={handleApplyFilter}
                />
