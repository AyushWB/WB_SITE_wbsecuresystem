  ...
  return (
    <MyContextProvider>
      ...
      <Head>
        ...
        {eventManager?.profile_image && <link rel="preload" as="image" href={eventManager.profile_image} />}
      </Head>
      ...
      <Layout>
        ...
        {chatOpen && (
          <div ref={chatBoxRef} style={chatBoxStyle} aria-live="polite">
            {showContent && (
              <>
                <div style={{ backgroundColor: "#870808", color: "#fff", padding: "10px 15px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center" }}>
                    ...
                    />
                    <div>
                      <strong style={{ fontSize: "13px" }}>{eventManager?.name || "Dolly"}</strong>
                      <p style={{ margin: 0, fontSize: "13px" }}>Event Manager</p>
                    </div>
                  </div>
                  <button onClick={handleCloseChat} style={{ background: "none", border: "none", color: "#fff", fontSize: "18px", cursor: "pointer" }} aria-label="Close chat">✕</button>
                </div>
                <div style={{ padding: "15px" }}>
                  <p style={{ margin: 0, fontSize: "11px" }}>Hi! I'm {eventManager?.name || "Dolly"}, your wedding planning assistant!</p>
                  <a href={`https://api.whatsapp.com/send?phone=918882198989&text=Hi%20${eventManager?.name || ""}`} target="_blank" rel="noopener noreferrer" style={{ display: "block", marginTop: "15px", textAlign: "center", backgroundColor: "#870808", color: "#fff", padding: "10px", borderRadius: "5px", textDecoration: "none", fontSize: "12px" }}>
        ...
        <div onClick={handleWhatsAppClick} className="whatsapp-icon" style={whatsappFabStyle} role="button" aria-label="Open WhatsApp chat">
          <Image
            ...
            loading="lazy"
          />
          {notificationCount > 0 && <div style={notifDotStyle} aria-label={`${notificationCount} new message`}>{notificationCount}</div>}
        </div>
